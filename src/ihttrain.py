...
def iht_train(sess,
    ...
    last_sparse_itr = 0
    num_epochs = (meta_iters - sparse_iter) / sparse_interval
    incre_rate = (compress_rate - init_rate) / (num_epochs - 1)
    ...
    for i in tqdm(range(meta_iters), desc="Training Phase"):
        ...
        # Judge whether using ITH or not.
        if ((i >= sparse_iter) and (i % sparse_interval==0)):
            ...
            cur_rate = init_rate + ((i - sparse_iter) / sparse_interval) * incre_rate
            mask = mask_class.mask_generator(current_params, cur_rate)
            print("Mask Process Done. Current pruning rate is: {:.4f}".format(cur_rate))
            #print("The mask is {}".format(mask[0]))
            reptile.get_mask(mask)
            processed_params = update_params(current_params, mask)
            model_state.import_variables(processed_params)

        if ((i >= sparse_iter) and ((i - last_sparse_itr) / sparse_interval < ratio)):
            reptile.update_sparse_signal(True)
        # restore the truncted params
        else:
